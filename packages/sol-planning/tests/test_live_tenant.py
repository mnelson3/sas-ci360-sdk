...
# -*- coding: utf-8 -*-
"""
UAT / integration tier for sol-planning (TRD 2.9): exercises the client
...
@pytest.fixture
def client():
    config = CI360PlanningConfig(host=_full_host(), secret_key=_SECRET_KEY, tenant_id=_TENANT_ID)
    return CI360PlanningBase(config)
...
def test_get_campaigns_returns_a_real_response(client):
    result = client.get_campaigns(limit=1)

    assert isinstance(result, dict)


def test_get_audiences_returns_a_real_response(client):
    result = client.get_audiences(limit=1)
