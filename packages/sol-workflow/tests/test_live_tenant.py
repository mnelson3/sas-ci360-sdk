...
# -*- coding: utf-8 -*-
"""
UAT / integration tier for sol-workflow (TRD 2.9): exercises the client
...
@pytest.fixture
def client():
    config = CI360WorkflowConfig(host=_full_host(), secret_key=_SECRET_KEY, tenant_id=_TENANT_ID)
    return CI360WorkflowBase(config)
...
def test_get_workflows_returns_a_real_response(client):
    result = client.get_workflows(limit=1)

    assert isinstance(result, dict)


def test_get_workflow_templates_returns_a_real_response(client):
    result = client.get_workflow_templates(limit=1)
