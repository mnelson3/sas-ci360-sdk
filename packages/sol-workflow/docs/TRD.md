...
| --- | --- | --- |
| FR-1 | List/get/create/update/delete workflows (with filters on list). | `get_workflows`, `get_workflow`, `create_workflow`, `update_workflow`, `delete_workflow` (+ `_async`) |
| FR-2 | Start a process; get its status; cancel it; list processes (status filter). | `start_process`, `get_process_status`, `cancel_process`, `get_processes` (+ `_async`) |
| FR-3 | List/create/update/delete triggers (with filters on list). | `get_triggers`, `create_trigger`, `update_trigger`, `delete_trigger` (+ `_async`) |
| FR-4 | List workflow templates (category filter); create a workflow from a template. | `get_workflow_templates`, `create_workflow_from_template` (+ `_async`) |
...
| ID | Status |
| --- | --- |
| NFR-1 (Auth correctness) | **Real bug found and fixed during the monorepo migration**: this package (and `sol-planning`) generated JWTs with the wrong payload shape and an unencoded signing key — CI360 would have rejected them outright, despite both passing their own mocked unit tests. Fixed by delegating to `api-core`'s canonical `Encryption` class, matching every other client. |
| NFR-4 (Testability, correct boundary) | **Fixed**: same `urljoin` api_base-dropping defect as `sol-data`, `sol-execute`, and `sol-identity`. Fixed to plain string formatting. |
...
Same NFR-8 pattern as every `sol-*` package; this package's `requirements.txt`/`setup.cfg` were also missing `pandas`/`saspy`/`schedule`/`DateTime` (the transitive deps every sibling redundantly re-declares) until 2026-09-20 — see the parent TRD's NFR-8 note. Fixed.
