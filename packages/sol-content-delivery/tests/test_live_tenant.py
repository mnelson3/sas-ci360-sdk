...
# -*- coding: utf-8 -*-
"""
UAT / integration tier for sol-content-delivery (TRD 2.9): exercises the
...
@pytest.fixture
def client():
    config = CI360ContentDeliveryConfig(host=_full_host(), secret_key=_SECRET_KEY, tenant_id=_TENANT_ID)
    return CI360ContentDeliveryBase(config)
...
def test_get_assets_returns_a_real_response(client):
    result = client.get_assets(limit=1)

    assert isinstance(result, dict)


def test_get_content_templates_returns_a_real_response(client):
    result = client.get_content_templates(limit=1)
