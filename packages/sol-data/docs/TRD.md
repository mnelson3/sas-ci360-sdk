...
| --- | --- | --- |
| FR-1 | List/get/create/update/delete customers. | `get_customers`, `get_customer`, `create_customer`, `update_customer`, `delete_customer` (+ `_async`) |
| FR-2 | List/get/create/update/delete segments. | `get_segments`, `get_segment`, `create_segment`, `update_segment`, `delete_segment` (+ `_async`) |
| FR-3 | Bulk import/export/validate data. | `import_data`, `export_data`, `validate_data` (+ `_async`) |
| FR-4 | Get/update a data schema. | `get_schema`, `update_schema` (+ `_async`) |
| FR-5 | List/create/get import request jobs. | `get_import_request_jobs`, `create_import_request_job`, `get_import_request_job` (+ `_async`) |
| FR-6 | Create a signed upload URL and upload a local file to it directly (bypassing this client's normal auth headers, since the signed URL is itself pre-authenticated). | `create_file_transfer_location`, `upload_to_signed_url` (+ `_async`) |
| FR-7 | List/get/create/update/delete tables. | `get_tables`, `get_table`, `create_table`, `update_table`, `delete_table` (+ `_async`) |
...
`requirements.txt` re-declares `api-core`'s own dependencies (`pandas`, `saspy`, `schedule`, `DateTime`) redundantly — see [sas-ci360-sdk/docs/TRD.md](../../../docs/TRD.md) NFR-8 for why.
