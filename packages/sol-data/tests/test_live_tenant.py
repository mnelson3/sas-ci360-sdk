...
# -*- coding: utf-8 -*-
"""
UAT / integration tier for sol-data (TRD 2.9): exercises the client
...
@pytest.fixture
def client():
    config = CI360DataConfig(host=_full_host(), secret_key=_SECRET_KEY, tenant_id=_TENANT_ID)
    return CI360DataBase(config)
...
def test_get_tables_returns_a_real_response(client):
    result = client.get_tables(limit=1)

    assert isinstance(result, dict)


def test_get_customers_returns_a_real_response(client):
