...
| --- | --- | --- |
| SOLIDENTITY-FR-1 | List/get/create/update/PATCH/delete users. | `get_users`, `get_user`, `create_user`, `update_user`, `patch_user`, `delete_user` (+ `_async`) |
| SOLIDENTITY-FR-2 | List/get/create/update/delete groups. | `get_groups`, `get_group`, `create_group`, `update_group`, `delete_group` (+ `_async`) |
| SOLIDENTITY-FR-3 | Authenticate a user; validate/refresh a token. | `authenticate_user`, `validate_token`, `refresh_token` (+ `_async`) |
| SOLIDENTITY-FR-4 | Get the SCIM service-provider configuration. | `get_service_provider_config` (+ `_async`) |
| SOLIDENTITY-FR-5 | Perform a bulk SCIM operation. | `bulk_operation` (+ `_async`) |

`api_base` default: `/scim`.

## 2. Non-functional requirements

Inherits SOLIDENTITY-NFR-1 through SOLIDENTITY-NFR-8 from [sas-ci360-sdk/docs/TRD.md](../../../docs/TRD.md). Status specific to this package as of 2026-09-20:

| ID | Status |
| --- | --- |
| SOLIDENTITY-NFR-4 (Testability, correct boundary) | **Fixed, twice over** — the only package where the urljoin defect appeared in two places: `_make_request_async`'s main URL construction, and `validate_connection_async`'s SCIM `ServiceProviderConfig` health check (via the absolute-path-reference variant of the same defect — `urljoin` also replaces the whole path when its second argument is itself absolute). Both fixed with plain string formatting. |
...
Same as the parent repository, minus `saspy`/`pandas`/`schedule`/`DateTime` for unit testing (see SOLIDENTITY-NFR-4 above) — those remain real production dependencies via `requirements.txt`, just not needed to run this package's own tests.

## 5. Dependency policy

See SOLIDENTITY-NFR-4 above — this package's `requirements-dev.txt` is the reference example in this repository for testing without a private dependency installed.
