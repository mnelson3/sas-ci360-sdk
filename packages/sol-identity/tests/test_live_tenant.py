...
# -*- coding: utf-8 -*-
"""
UAT / integration tier for sol-identity (TRD 2.9): exercises the client
against a real CI360 tenant's SCIM endpoint instead of a mocked one.
Every test here is skipped, not failed, unless CI360_HOST,
CI360_SECRET_KEY, and CI360_TENANT_ID are all set - so this file is
always safe to collect, including in the default CI job, where those
...
@pytest.fixture
def client():
    config = CI360IdentityConfig(host=_full_host(), secret_key=_SECRET_KEY, tenant_id=_TENANT_ID)
...
def test_get_users_returns_a_real_response(client):
    result = client.get_users(limit=1)

    assert isinstance(result, dict)


def test_get_service_provider_config_returns_a_real_response(client):
