...
class Events(Base):
    ...
    def create_bulk_events(self, payload: dict) -> requests.Response:
        ...
        :return: Submits a request to upload a file with external events. When you send
            this request with the appropriate JSON body, the response includes a signed,
            temporary URL. Use this URL to upload a bulk events file.
    ...
    def create_external_event(self, payload: dict) -> requests.Response:
        ...
        :return: Inject an external event into SAS Customer Intelligence 360. Define
            the external event in the JSON body of the request.
        :rtype: requests.Response
        """
        token = self.token
        api = self.api
