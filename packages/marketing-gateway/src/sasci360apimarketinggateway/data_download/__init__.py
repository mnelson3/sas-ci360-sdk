...
class DataDownload(Base):
    ...
    Contains the operations to download data tables for customer behavior,
    identities, metadata, and so on. The tables that you have access to are
    based on the licenses you have and the data that is available in the tables
    ...
    def get_base_tables(self, **kwargs) -> requests.Response:
        ...
        :keyword date_range_start_time_stamp: str, optional - The start timestamp to use.
            The timestamp must be in UTC format: YYYY-MM-DDThh:mm:ss.sssZ
        :keyword date_range_end_time_stamp: str, optional - The end timestamp to use.
            The timestamp must be in UTC format: YYYY-MM-DDThh:mm:ss.sssZ
        :keyword schema_version: int, optional - The schema that the tables use
        :return: Returns the links to download the Discover Base Tables (DBTs) and
            Analytical Base Tables (ABTs). These tables are updated about every four
            hours. Only completed sessions are available for download.
    ...
    def get_detail_tables(self, **kwargs) -> requests.Response:
        ...
        :keyword date_range_end_time_stamp: str, optional - The end timestamp to use.
            The timestamp must be in UTC format: YYYY-MM-DDThh:mm:ss.sssZ
        :keyword include_all_hour_status: bool, optional - Specifies whether to download
            all data in the requested range, regardless of a set's internal status
        :keyword sub_hourly_data_range_in_minutes: int, optional - The number of minutes
            that are contained in each partition. Typically, you set this value from 10 up to 60
        :keyword schema_version: int, optional - The schema that the tables use
        :keyword category: str, optional - When you download tables with schema 4 or later,
            this value corresponds to the set of tables to be downloaded. When the
            parameter is not set, the default value is DISCOVER
        :return: Returns links to download the Detail tables, which contain the most
            detailed set of data that is available for user interactions. These tables
            also contain the source records that are processed and combined to make up
            reporting and Discover base tables (DBTs).
        :rtype: requests.Response
        """
        token = self.token
        api = self.api
        ...
        params = None
        api_path = "/discoverService/dataDownload/eventData/detail/partitionedData{0}".format(
            self._finish_query_string(query_string))
        url = "https://{0}{1}{2}".format(host, api, api_path)
        return self.connection.connect(action=action, data=data, headers=headers, params=params, url=url)
    ...
    def get_identity_tables(self, **kwargs) -> requests.Response:
        ...
        :keyword category: str, optional - When you download tables with schema 4 or later,
            this value corresponds to the set of tables to be downloaded. When the
            parameter is not set, the default value is DISCOVER
        :return: Returns links to download the identity tables, metadata tables, and
            plan tables from the system.
        :rtype: requests.Response
        """
        token = self.token
        api = self.api
        ...
        params = None
        api_path = "/discoverService/dataDownload/eventData/detail/nonPartitionedData{0}".format(
            self._finish_query_string(query_string))
        url = "https://{0}{1}{2}".format(host, api, api_path)
        return self.connection.connect(action=action, data=data, headers=headers, params=params, url=url)
    ...
    def get_reprocessed_tables(self, **kwargs) -> requests.Response:
        ...
        :return: Returns links to download data sets that have already been reprocessed
            by the system (but not those ranges that are currently being reprocessed).
            Data can be reprocessed because a job failed or if the system specifically
            requested that a data set be reprocessed.
        :rtype: requests.Response
        """
        ...
        params = None
        api_path = "/discoverService/dataDownload/eventData/partitionedData/resets{0}".format(
            self._finish_query_string(query_string))
        url = "https://{0}{1}{2}".format(host, api, api_path)
        return self.connection.connect(action=action, data=data, headers=headers, params=params, url=url)
